...
async function latestIOSVersion(
  ...
  // Consumer catalogs can contain an app missing from the enterprise listing.
  // Keep the account's storefront and require an iOS offer in each catalog.
  for (const platform of ['ios', 'iphone', 'ipad']) {
  ...
  throw new DownloadError(
    `No iOS version available for app ${app.id} in storefront ${country} (ios, iphone, ipad)`,
  );
}
